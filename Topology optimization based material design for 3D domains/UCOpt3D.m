function UCOpt3D(lx,ly,lz,nelx,nely,nelz,nlx,nly,nlz,volfrac,penal,rmin,p1,p2)
...
while change > tolx && loop < maxloop
    ...
    % Homogenization
    [CH,DCH] = homo3DY(0.000001*lx,0.000001*ly,0.000001*lz,E,dE,nu,Emin,h2,p2);
    % FE-ANALYSIS
    KE = h1.elemStiffness(CH);
    sK = reshape(KE(:)*ones(1,nely*nelx*nelz),576*nelx*nely*nelz,1);
    K = sparse(iK(:),jK(:),sK(:)); K = (K+K')/2;
    U(freedofs,:) = p1.solve(K(freedofs,freedofs),F(freedofs,:));
