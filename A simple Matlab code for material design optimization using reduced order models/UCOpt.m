function c = UCOpt(lx,ly,nelx,nely,nlx,nly,volfrac,penal,rmin,ft,p)
%% MATERIAL PROPERTIES
E0 = 1;
Emin = 1e-9;
nu = 0.3;
...
while change > 0.01
  ...
  %% FE-ANALYSIS
  KE = Q4elementStiffnessMatrix(lx/nelx/2,ly/nely/2,90,CH);
...
%   U(freedofs) = K(freedofs,freedofs)\F(freedofs);
  ...
  for i = 1:nly
      for j = 1:nlx
          dKE = Q4elementStiffnessMatrix(lx/nelx/2,ly/nely/2,90,DCH{i,j});
          ce = reshape(sum((U(edofMat)*dKE).*U(edofMat),2),nely,nelx);
          c = c + sum(sum(ce));
          dc(i,j) = - sum(sum(ce));
      end
  end
...
% based on the top88.m code written by E. Andreassen, A. Clausen, 
% M. Schevenels, B. S. Lazarov and O. Sigmund presented in the 
% paper "Efficient topology optimization in MATLAB using 88 lines of code",
% E. Andreassen, A. Clausen, M. Schevenels, B. S. Lazarov and O. Sigmund,
% Struct Multidisc Optim, 2010 
%
