function [c,CH,x,y] = ConcTopOptBESO(lx,ly,nelx,nely,nlx,nly,volfx,volfy, ...
...
while change > 0.001 && loop < 200
  ...
  %% SOFT-BESO UPDATE OF DESIGN VARIABLES MACRO
  l1 = min(min(dc)); l2 = max(max(dc)); 
  while (l2-l1)/(l1+l2) > 1e-3
    lmid = 0.5*(l2+l1);
    x = max(0.001,sign(dc-lmid));
    if sum(x(:)) > volx*nelx*nely, l1 = lmid; else, l2 = lmid; end
  end
  %% SOFT-BESO UPDATE OF DESIGN VARIABLES MICRO
  l1 = min(min(dcy)); l2 = max(max(dcy));
  while (l2-l1)/(l1+l2) > 1e-3
    lmid = 0.5*(l2+l1);
    y = max(0.001,sign(dcy-lmid));
%     if sum(y(:)) > voly*nlx*nly, l1 = lmid; else, l2 = lmid; end
    if sum(y(:)+(1 - y(:))*1/8) > voly*nlx*nly, l1 = lmid; else, l2 = lmid; end
  end
  if loop > 10
