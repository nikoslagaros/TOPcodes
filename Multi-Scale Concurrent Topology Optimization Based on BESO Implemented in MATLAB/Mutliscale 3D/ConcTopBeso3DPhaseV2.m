function [c,CH,x,y,fx,fy] = ConcTopBeso3DPhaseV2(lx,ly,lz,nelx,nely,nelz,nlx,nly,nlz, ...
    volf,penal,rxmin,rymin,er,p)
...
E2 = 1.5;      % Young's modulus of void-like material
xmin = 1e-8;
...
while change > tolx && loop < maxloop
    ...
    % INTERPOLATION
    [E,dE] = interpolate(y,E0,E2,penal);
    % Homogenization
    ...
    % BESO UPDATE
    l1a = min(dc(:)); l1b = min(dcy(:)); l1 = min(l1a,l1b);
    l2a = max(dc(:)); l2b = max(dcy(:)); l2 = max(l2a,l2b);
    while (l2-l1)/(l1+l2) > 1e-4
        lmid = 0.5*(l2+l1);
        x = max(0.001,sign(dc-lmid));
        y = max(0.001,sign(dcy-lmid));
        if sum(x(:))/nele * sum(y(:)+(1-y(:))*r)/nlx/nly/nlz > vol, l1 = lmid; else, l2 = lmid; end
    end
    if loop > 10
        change  = abs(sum(c(loop-9:loop-5))-sum(c(loop-4:loop)))/sum(c(loop-4:loop));
    end
    fx(loop) = mean(x(:));
    fy(loop) = mean(y(:)+(1-y(:))*r);
    % PRINT RESULTS
    fprintf(' It.:%5i Obj.:%11.4f Volx.:%7.3f Voly.:%7.3f ch.:%7.3f\n', ...
        loop,c(loop),mean(x(:)),mean(y(:)+(1-y(:))*r),change);
    % PLOT DENSITIES
    if displayflag, figure(1); clf; display_3D(x); end
    if displayflag, figure(2); clf; display_3D_V2(y); end
end
figure(1); clf; display_3D(x);
figure(2); clf; display_3D_V2(y);
end